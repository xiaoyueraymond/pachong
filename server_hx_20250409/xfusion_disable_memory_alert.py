...
class xFusion_redfish:
    ...
    def __init__(self, args):
        ...
        self.parse_alarm()
        self.disable_memory_alert('0x01000073')
        self.disable_memory_alert('0x29000001')
        # self.get_cpu_info()
    ...
    def get_detail_memory(self, endpoint):
        ...
        if not all((locator_rsp, capacity_rsp, speed_rsp, rank_rsp)):
            ...
            msg = ('\t').join((locator_rsp, capacity_rsp, speed_rsp, rank_rsp))
            mf.display(msg)
            raise Exception(msg)
    ...
    def get_cpu_info(self):
        ...
        for cpu in cpu_dict:
            ...
            if expected_cpu_check:
                ...
                #
                if (expected_cpu_check_dict['expected_cpu_model'] in model) and \
                        (expected_cpu_check_dict['expected_cpu_core'] == cores) and \
                        (expected_cpu_check_dict['expected_cpu_thread'] == total_threads):
                    ...
                    mf.display(msg)
                    msg1 = f"Model:{model} \t Cores:{cores}\tThreads:{total_threads}"
                    mf.display(msg1)
                    raise Exception(msg)
    ...
    def get_pci_devices(self):
        ...
        if num:
            pci_tables = []
            headers = ['Name', 'Description', 'Manufacturer', 'FirmwareVersion', 'DeviceLocator', 'Position']
    ...
    def get_pci_detail(self, url):
        ...
        if expected_pcie_check:
            ...
            if expected_pcie_check_dict[Name]:
                ...
                return [Name, Description, Manufacturer, FirmwareVersion, DeviceLocator, Position]
    ...
    def post_content(self, url, body=None, ):
        rsp = self.client.post(url, body=body).dict
        return rsp

    def disable_memory_alert(self,event_code):
        """
        内存事件码： 0x01000073
        true: 开启屏蔽
        false: 取消屏蔽
        """
        # url = '/redfish/v1/EventService/Actions/Oem/xFusion/EventService.ShieldSystemAlert'
        url = '/redfish/v1/EventService/Actions/Oem/xFusion/EventService.ShieldSystemAlert'
        body = {
            "EventCode": event_code,
            # "EventCode": "0x01000073",
            "Enabled": True}
        # nbody = json.dumps(body)
        print(self.client._get_req_headers())
        rsp = self.client.post( url, body=body)
        # pdb.set_trace()
        time.sleep(5)
        mf.display("在测试关闭事件后的状态")
