...
class xFusion_redfish:
    ...
    def __init__(self, args):
        ...
        self.get_kvm_service()
        self.patch_kvm_settings()
    ...
    def get_kvm_service(self):
        url = '/redfish/v1/Managers/1/KvmService'
        content = self.client.get(url).dict
        for k,v in content.items():
            print(k,'==>',v)

    def patch_kvm_settings(self):
        json_body =""" {
            "EncryptionEnabled": true
        }"""
        body = json.loads(json_body)
        url = "/redfish/v1/Managers/1/KvmService"
        headers = self.client._get_req_headers()
        headers["Content-Type"] = "application/json"
        h = json.dumps(headers)
        content = self.client.patch(url,body=body,headers=h)
        pdb.set_trace()
        for k,v in content.items():
            print(k,'==>',v)
