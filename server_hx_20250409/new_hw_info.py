...
def log_memory():
    h = ['Size', 'Locator', 'Speed', 'Manufacturer', 'Serial Number', 'Part Number']
    mem_tables = []
    for mem in mem_info:
        mem_list = [mem['Size'], mem['Locator'], mem['Speed'], mem['Manufacturer'], mem['Serial Number'], mem['Part Number']]
        mem_tables.append(mem_list)
    mf.display(mf.create_section("Logging MEM info"))
    mf.display(mf.create_table(h,mem_tables))

def log_cpu():
    """
    add single cpu check with status
...
Processor Information
        ...
        Status: Unpopulated   -------------------- not install ,empty socket ,updated
    ...
    header = ['Socket',  'Version', 'Core Count' ,   'Thread Count' ]
    #header = "{}\t{}\t{}\t{}".format('Socket',  'Version', 'Core Count' ,   'Thread Count' )
    cpu_tables = []
    #logging.info(header)
    for cpu in cpu_info:
        if cpu.get('Status') == 'Unpopulated':
            break
        cpu_tables.append([cpu['Socket Designation'],  cpu['Version'], cpu['Core Count'] ,   cpu['Thread Count'] ])
        #msg = "{}\t{}\t{}\t{}".format(cpu['Socket Designation'],  cpu['Version'], cpu['Core Count'] ,   cpu['Thread Count'] ) 
        #logging.info(msg)
    #log_line(_type=0)
    mf.display(mf.create_section("CPU　Information"))
    mf.display(mf.create_table(header,cpu_tables))
...
def log_ocp():
    ...
    header = ['Reference Designation', 'Type','Bus Address', 'Physical Slot','LnkSta','Product Name']
    ...
    #logging.info(header)
    mf.display(mf.create_section("OCP Adapter Information"))
    ocp_tables = []
    if not ocp_info:
        ...
        for record in ocp_info:
            ...
            ocp_tables.append([record['Reference Designation'], record['Type'],record['Bus Address'],record[ 'Physical Slot'],link_status,record['Product Name']])
        mf.display(mf.create_table(header,ocp_tables))
...
2a:00.0 Fibre Channel: QLogic Corp. ISP2684 (rev 01)
    ...
    """
    mf.display(mf.create_section("Check QLogic FC　Card"))
    content = get_qlogic_fc_info()
    header = ['Fibre Channel','Physical Slot','LnkSta','Part number','Serial number']
    #logging.info(header)
    fc_lists = []
    if not content:
        ...
        for info in content:
            ...
            if is_downgraded(link_status):
                link_status = '\033[31m' + link_status + "\033[0m"
            fc_lists.append([info['Fibre Channel'],info['Physical Slot'],link_status,info['[PN] Part number'],info['[SN] Serial number']])
        table = mf.create_table(header,fc_lists)
        mf.display(table)
...
def log_nvidia_gpu_info():
    ...
    """
    mf.display(mf.create_section("Check Nvidia GPU Info"))
    gpu_info = get_nvidia_gpu_info()
    header = ['GPU Card', 'Physical Slot', 'LnkSta', 'Part number', 'Serial number']
    gpu_lists = []
    ...
    if not content:
        ...
        for info in content:
            link_sta = info['LnkSta']
            if is_downgraded(link_sta):
                link_status = '\033[31m' + link_sta + "\033[0m"
                logging.warning(link_status)
            gpu_card =  info.get('3D controller','GPU Card')
            gpu = [ gpu_card, info['Physical Slot'], link_sta, "To be defined ", "To be defined"]
            gpu_lists.append(gpu)

        gpus = mf.create_table(header,gpu_lists)
        mf.display(gpus)
