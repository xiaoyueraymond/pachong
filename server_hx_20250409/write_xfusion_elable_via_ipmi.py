...
parser.add_argument('-s', '--sn', help='specify the port to the target iBMC, default is 443')
...
#     cmd = f"ipmitool  -I lanplus  -H {args.host} -U {args.username} -P {args.password}  " + elabelGetCmd[
#         command] + " 0x30"
#     mycmd = cmd.split()
#     rsp = run(mycmd)
#     mf.display(rsp)
...
# 写
def write_product_serial_number(info):
    """
    info: str of the serial number ,will change to hex format
    """
    str_len = len(info)
    msg = f'update the product serial number to {info}, it\'s length is {str_len}'
    mf.display(msg)
    if not info.startswith('2106'):
        msg = "错误的信息，请修改，序列号以2106开始"
        mf.display(msg)
        raise Exception(msg)
    #
    hex_info = str2hex(info)
    write_product_sn = 'raw 0x30 0x90 0x4 0x0 0x3 0x4 0x00 '
    cmd =  f"ipmitool  -I lanplus  -H {args.host} -U {args.username} -P {args.password}  " + write_product_sn +  str_len  + " " + hex_info
    mycmd = cmd.split()
    run(mycmd)
    msg = f"update product SN to {info} successfully  "
    mf.display(msg)


def main():
    if args.sn:
        write_product_serial_number(args.sn)