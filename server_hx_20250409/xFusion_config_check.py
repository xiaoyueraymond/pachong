...
parser.add_argument('-C','--check',dest='check',choices=[True,False],default=True,
                    help="check the HW config with Config file")
parser.add_argument('-F','--file',dest='file',default='./xFusion_Config.py',
                    help="Specify the Hardware Config file to check compaere  the real config in Server")


args = parser.parse_args()

config_check = args.check
if config_check:
    ...
    if os.path.exists(config_file):
        ...
        msg = "Can not find the config ,please check"
        mf.display(msg)
        msg = "找不到指定的配置文件，请检查或者联系开发"
        pdb.set_trace()
#
pdb.set_trace()
...
cmd = "echo 123456 >d:\\xfusion_redfish.log"
os.system(cmd)
...
class xFusion_redfish:
    ...
    def get_memory_summary(self):
        ...
        #
        #
        if  not ( expected_memory_counts == self.mem_counts ):
            msg = f"expected memory is :\t{expected_memory_counts} pcs,\t but we only find Total memory:\t {self.mem_counts}  pcs in this server"
            mf.display(msg)
            pdb.set_trace()
        #
        if expected_memory_brand_check:
            if not all([x[2] == self.mem_list[0][2] for x in self.mem_list]):
                msg = "memory installed with different brand ,please check and fixed it "
                mf.display(msg)
                pdb.set_trace()
        #
    ...
    def get_detail_memory(self, endpoint):
        ...
        expected_locators = expected_memory_locators
        '''
        ...
        if (contents['DeviceLocator'] not in expected_locators) or (contents['CapacityMiB'] not in expected_memory_size ) or ( contents['OperatingSpeedMhz'] != expected_memory_speed ):
            msg1 = " 内存装配错误，请检查 "
            mf.display(msg1)
            pdb.set_trace()
    ...
    def get_cpu_info(self):
        ...
        if self.cpu_counts != expected_cpu_counts:
            msg = f"CPU installed counts is mismatched, expect to {expected_cpu_counts},but only { self.cpu_counts} CPU　installed"
            mf.display(msg)
            pdb.set_trace()
    ...
    def get_detail_cpu(self, url):
        ...
        if expected_cpu_model not in Model:
            msg = "expected CPU is {expected_cpu_model },but installed model is {Model}"
            mf.display(msg)
            pdb.set_trace()
    ...
    def parse_psu(self):
        ...
        for psu in psu_dict:
            ...
            pn = psu['PartNumber']
            if expected_psu_watts != PowerCapacityWatts:
                msg = f"mismatch power capacity watts. please double check !"
                mf.display(msg)
                pdb.set_trace()
            if expected_psu_Model != Model:
                msg = f"mismatch power model.expected is {expected_psu_Model},but {Model}  Installed, please double check !"
                mf.display(msg)
                pdb.set_trace()
        ...
        """
        if expected_psu_counts != self.psu_counts:
            msg = f"PSU Installed count is mismatched ,expected is {expected_psu_counts},but we only found {self.psu_counts} PSU Installed"
            mf.display(msg)
            pdb.set_trace()
        ...
        if rsp.get('Oem', None):
            if rsp['Oem'].get('xFusion', None):
                if 'HealthEvent' in rsp['Oem']['xFusion'].keys():
                    ...
                    msg = "no Keyword :HealthEvent found or no alarm happen "
                    mf.display(msg)
            else:
