...
class xFusion_redfish:
    ...
    def get_info(self):
        ...
        pa = rsp['Oem']['xFusion']['ProductAlias']
        msg = f"get Cuutent product Alias value :\t    {pa}"
        mf.display(msg)

    def set_memory_sn_Severity(self, ):
        url = '/redfish/v1/RegistryStore/Messages/en/iBMCEvents.v11_22_11.json'
        body = {'Messages': {'memorySnAuthInvalid': {'Severity': 'Info'}}}
        # pdb.set_trace()
        # self.etag
        headers = {'If-Match': self.etag}
        #rsp = self.client.patch(url, body=body,headers=headers).dict
        rsp = self.client.patch(url, body=body,headers=headers)

        pdb.set_trace()
        pa = rsp['Messages']['memorySnAuthInvalid']['ProductAlias']
        msg = f"Set Memory SN authentication Severity to Info : {pa}"
        mf.display(msg)
        time.sleep(1)
        # pdb.set_trace()
    ...
    def get_content(self, url):
        ...
        self.etag= dict(rsp.getheaders()).get('ETag')
        return rsp.dict
    ...
    def patch_content(self, *args, **kwargs):
