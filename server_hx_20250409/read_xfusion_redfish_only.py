...
import logging
from mytools import *
from xfusion_config_read_only import *
...
parser.add_argument('-H', '--host', dest='host', default='192.168.0.19',
...
logfile = logdir + 'xfusion_config_only.log'
...
class xFusion_redfish:
    ...
    def get_pci_devices(self):
        ...
        if num:
            ...
            mf.display(tables)
        if not expected_pcie_check:
            self.pci_msg = f"{num} PCI device installed"
            return
    ...
    def parse_psu(self):
        ...
        if not expected_psu_check:
            self.psu_msg  = f"{len(psu_tables)} PSU  installed"
            return
            ...
            if rsp['Oem'].get('xFusion', None):
                if 'HealthEvent' in rsp['Oem']['xFusion'].keys():
                    ...
                    if alert_num:
                        ...
                        mf.display(tables)
                        #
                        # pdb.set_trace()   ## display the alarm message for current machines
                    else:
...
target_file = logdir + 'xFusion_read_config_only_' + sn + '.log'
if os.name == 'nt':
