...
parser.add_argument('-P', '--PASS', dest='password', default='WKWP4YVE9EXP',
                    help='specify the password to use to access the iBMC')
parser.add_argument('-H', '--host', dest='host', default='192.168.0.7',
...
class xFusion_redfish:
    ...
    def get_detail_memory(self, endpoint):
        ...
        if not all((locator_rsp, capacity_rsp, speed_rsp, rank_rsp)):
            ...
            mem_result = [ str(item) if isinstance(item, bool) else item for item in (locator_rsp, capacity_rsp, speed_rsp, rank_rsp)]
            msg = ('\t').join(mem_result)
            mf.display(msg)
            raise Exception(msg)
    ...
    def get_cpu_summary(self):
        msg = "get cpu summary "
        mf.display(mf.create_section(msg))
        cpu_url = '/redfish/v1/Systems/System.Embedded.1/Processors/'
        rsp = self.client.get(cpu_url).dict
        # pdb.set_trace()
        cpu_counts = len(rsp['Members'])
    ...
    def get_detail_cpu(self, url):
        ...
        Model = rsp['Model']
        FrequencyMHz = rsp['Oem']['Dell']['FrequencyMHz']
            ...
            if expected_pcie_check_dict[Name]:
                ...
                if not all(check_result):
                    ...
                    if not check_result[1]:
                        ...
                        # check_pci_result = "FAILED"
                    # pdb.set_trace()
                    elif not check_result[2]:
                        # check_pci_result = "FAILED"
                        msg = f"wrong PCIE card positions:{Name}"
                        mf.display(msg)
                    else:
                        # check_pci_result = "FAILED"
                        msg = f"wrong Card installed"
                        mf.display(msg)
