...
def gate2():
    ...
    filePath = dir + "/history2.json"
    if os.path.exists(filePath):
        with open(filePath, "r", encoding="utf-8") as file:
            history = json.loads(file.read())
        if articles[0]["show_pair"] != history[0]["show_pair"]:
            if int(articles[0]["buy_start"]) < time.time():
                return
            date = time.strftime(
                timeFormat, time.localtime(int(articles[0]["buy_start"]))
            )
            title = "检测到新币 " + articles[0]["show_pair"] + " 将上线"
            link = "https://www.gate.io/zh/pilot" + articles[0]["seo_path"]
            isSend = True
            for name in filterList:
                if name in title:
                    isSend = False
...
def main():
    while True:
        ...
        threading.Thread(target=gate2).start()
