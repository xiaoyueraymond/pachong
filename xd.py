...
def mexc_gate():
    ...
    while True:
        try:
            ...
            if  (order_book_gate['asks'][3][0]) < price_mexc *  chazhi  :
            ...
            elif  order_book_mexc['asks'][3][0] < price_gate *  chazhi  :
                sell_running_mexc = True
                logger.debug(f'{symbols} 价格相差 {x}倍 (MEXC: {price_mexc}, GATE: {price_gate}),mexc启动买入程序')  
                try:
            ...
            elif  order_book_mexc['asks'][2][0] < price_gate *  chazhi  :
                sell_running_mexc = True
                logger.debug(f'{symbols} 价格相差 {x}倍 (MEXC: {price_mexc}, GATE: {price_gate}),mexc启动买入程序')  
                try:
                    order_cancel = exchange_mexc.cancel_all_orders(symbols)
            ...
            elif  order_book_mexc['asks'][1][0] < price_gate *  chazhi  :
                sell_running_mexc = True
                logger.debug(f'{symbols} 价格相差 {x}倍 (MEXC: {price_mexc}, GATE: {price_gate}),mexc启动买入程序')  
                try:
                    order_cancel = exchange_mexc.cancel_all_orders(symbols)
            ...
            elif  order_book_mexc['asks'][0][0] < price_gate *  chazhi  :
                sell_running_mexc = True
                logger.debug(f'{symbols} 价格相差 {x}倍 (MEXC: {price_mexc}, GATE: {price_gate}),mexc启动买入程序')  
                try:
                    order_cancel = exchange_mexc.cancel_all_orders(symbols)
                ...
                logger.debug(f'{symbols} 价格未达到 {chazhi * 100:.2f}% (MEXC: {price_mexc}, GATE: {price_gate})，差值：{x}')                  
        except Exception as e:
...
def mexc_bitget(symbols):
    while True:
        try:
            ...
            if  (order_book_bitget['asks'][3][0]) < price_mexc *  chazhi  :
            ...
            elif  (order_book_bitget['asks'][2][0]) < price_mexc *  chazhi  :
                price_difference = abs((price_mexc - price_bitget) / price_bitget) * 100
                logger.debug(f'{symbols} 价格相差 {x}倍 (MEXC: {price_mexc}, bitget: {price_bitget}),bitget启动买入程序{price_bitget}')
                # logger.debug('{} 价格相差10%'.format(symbols)) 
                try: 
            ...
            elif  (order_book_bitget['asks'][1][0]) < price_mexc *  chazhi  :
                price_difference = abs((price_mexc - price_bitget) / price_bitget) * 100
                logger.debug(f'{symbols} 价格相差 {x}倍 (MEXC: {price_mexc}, bitget: {price_bitget}),bitget启动买入程序{price_bitget}')
                # logger.debug('{} 价格相差10%'.format(symbols)) 
                try: 
            ...
            elif  (order_book_bitget['asks'][0][0]) < price_mexc *  chazhi  :
                price_difference = abs((price_mexc - price_bitget) / price_bitget) * 100
                logger.debug(f'{symbols} 价格相差 {x}倍 (MEXC: {price_mexc}, bitget: {price_bitget}),bitget启动买入程序{price_bitget}')
                # logger.debug('{} 价格相差10%'.format(symbols)) 
                try: 
...
def mexc_gate_sell():
    ...
    while True:
        if x < 1.1 and sell_running_gate:
            try: 
...
#python3 1bijia.py
#这是2个交易所，一个交易所已经上线了，另外一个后面上线，如果后面上线的交易所比先上线的交易所价格低30%，就买入。
