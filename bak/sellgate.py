...
API_KEY = 'd9ebc00738f9942017b6ee37f7ad1861'
...
def sell_market():
    try:
        order_cancel = exchange.cancel_all_orders(symbols)
        print("当前时间:", datetime.now(beijing_tz).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]) 
        execute_trading_strategy(p=-3, q=20)
    except Exception as e:
        print("当前时间:", datetime.now(beijing_tz).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]) 
...
def wait_until_target_time(target_time):
    while True:
        ...
        elif current_time_str == target_time_str and now.microsecond // 1000 >= target_time.microsecond // 1000:
            sell_market()
            break
...
beijing_tz = pytz.timezone('Asia/Shanghai')
beijing_time = datetime.now(beijing_tz)
target_time = datetime.now(beijing_tz).replace(hour=11, minute=38, second=3, microsecond=10000)
